# Masume アプリアイコンの構成とビルドの仕組み

このドキュメントでは、Masume アプリケーションにおけるアイコン（ロゴ）の構成ファイル、開発・更新ワークフロー、およびビルド処理の仕組みについて解説します。

---

## 1. 概要 (Overview)

macOS アプリケーション (`Masume.app`) では、Finder や Dock、タスク切替画面で表示されるアイコンとして `.icns` 形式のマルチ解像度アイコンが使用されます。
...
| パス | 種類 | 役割・説明 |
| :--- | :--- | :--- |
| `MasumeAppIcon.icon/` | ソースフォルダ | **Icon Composer 用編集データ**。<br>`icon.json`（グラデーションやグラスエフェクト等の設定）と `Assets/image.png` を含む再編集用データ。 |
...
| `scripts/build-app.sh` | スクリプト | **アプリパッケージングスクリプト**。`Resources/AppIcon.icns` を `Masume.app` にコピーする。 |
...
`scripts/build-app.sh` を実行して `Masume.app` をパッケージングする際、アイコンの組み込みは事前生成された `Resources/AppIcon.icns` のコピーのみで行われます。
...
### 3.2 なぜ CLI ビルドで `MasumeAppIcon.icon` (`icon.json`) を直接処理しないのか？
...
そのため、再編集用のマスターデータとしては `MasumeAppIcon.icon` を維持しつつ、ビルドおよび CLI での運用は事前エクスポートした `AppIcon.icns` / `AppIcon.png` を使用する設計としています。
...
1. Icon Composer (macOS Sequoia / Xcode 16) で `MasumeAppIcon.icon` またはデザインファイルを開き編集します。
