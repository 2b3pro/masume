...
- **Provenance.** Upstream Kakico adopted the PolyForm Noncommercial License 1.0.0 on 2026-09-06. The model, renderer, and canvas remain Hiroki Takatsuka's code under those terms, so Masume as a whole is noncommercial: it may be built, used, modified, and redistributed freely for noncommercial purposes, and every copy must carry the upstream terms (`LICENSE-kakico`) or their URL. Masume's own contributions are MIT, and the MCP server in `mcp/` is a separate program carrying no Kakico code. Attribution stays in the README and LICENSE regardless of how far the code drifts.
- **Automation permission.** macOS treats the terminal or editor that launches `masume` as the responsible process. The receiving Masume app therefore needs a stable, non-ad-hoc designated requirement, and each host (such as iTerm or VS Code) needs its own Automation grant for Masume. The MCP server still spawns the bundled CLI rather than `osascript`, keeping the transport direct and errors machine-readable.
